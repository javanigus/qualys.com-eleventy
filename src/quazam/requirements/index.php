...
$requirements=array(
	...
	array(
		...
		$message),
	array(
		t('yii','LDAP extension'),
		true,
		extension_loaded("ldap"),
		'Qualys WCMS, CLdapUserIdentity',
		''),
	array(
		t('yii','PHP Composer'),
		true,
		($message=checkComposer()) === '',
		'SVN library',
		$message),
	array(
		t('yii','SVN library'),
		...
		$message),
	array(
		t('yii','Multibyte String extension'),
		true,
		extension_loaded("mbstring"),
		'',
		t('yii','This is required for international support.')),
);
...
function checkComposer() {
	$message = t('yii','PHP Composer is not installed');

	if (file_exists(dirname(__FILE__).'/../../../quazam/protected/composer.phar')) {
		$message = '';
	}
	return $message;
}
...
function checkSVN()
{
	$message = t('yii','VersionControl_SVN not installed');

	if (($message=checkComposer()) === '') {
